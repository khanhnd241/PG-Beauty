...
class RegisterScreen extends Component {
            ...
            .then((res) => {
              if (res.data.data.length == 0) {
                ...
                this.setState({
                  ...
                  dateView: moment(res.data.data[0].birthDate).format(
                    'DD/MM/YYYY',
                  ),
  ...
  register = (data) => {
    ...
    axios
      .post(API.URL + API.REGISTER, data)
      .then((response) => {
        this.setState({ loadingDialog: false });
        if (response.data.success.token) {
          let tokenFirebase = DATABASE.getTokenFirebase();
            if (tokenFirebase) {
  ...
  ClickRegister = async () => {
    ...
    if (this.validate() == true) {
      ...
      if (
        this.state.dateView.trim() != '' &&
        this.state.dateView != STRING.ENTER_DATE_OF_BIRTH
      ) {
      ...
      if (this.state.id == null || this.state.id == '') {
        ...
        axios
          .post(API.URL_API_KIOT + API.CUSTOMERS, data, config)
          .then((response) => {
            if (
        ...
        axios
          .put(
            API.URL_API_KIOT + API.CUSTOMERS + '/' + this.state.id,
            data,
            config,
          )
          .then((response) => {
        ...
        <KeyboardAwareScrollView>
          <View style={styles.container}>
            ...
            <TextInput
              ...
              onBlur={() => this.connect()}
              placeholder={STRING.ENTER_PHONE_INPUT}
            ...
            <TouchableOpacity
              ...
              style={styles.input_dob}>
              <Text style={{ flex: 7, color: COLOR.PLACEHODER }}>
                {this.state.dateView}
              </Text>
              <View style={{ flex: 1 }}>
